...
const PRIORITY_COLOR = (id) => (id >= 3 ? "bg-signal-warn" : "bg-signal-info");
const PRIORITY_DOT = (id) => (id >= 3 ? "bg-signal-warn" : "bg-signal-info");
...
export default function CategoryPendingCard({ category, tickets, systems }) {
  ...
  return (
    <div className={`rounded-lg border-t-2 ${ACCENT_BORDER[category.accent]} border-line bg-base-900 shadow-panel p-5 flex flex-col gap-4`}>
      ...
      <div className="rounded-md border border-line bg-slate-100 px-3 py-2 flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {systems.map((s) => {
            ...
            return (
              <span
                key={s.id}
                className="inline-flex items-center gap-1.5 rounded-md border border-line bg-slate-100 px-2 py-1 text-xs font-body text-ink-mid"
              >
                {s.label}
