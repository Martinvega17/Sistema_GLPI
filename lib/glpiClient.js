...
// GLPI incrusta las imágenes pegadas/adjuntas en la descripción y en las
// soluciones como <img src="/front/document.send.php?docid=NN">. Esa URL
// solo es accesible con una sesión de GLPI activa (cookie de sesión web),
// que el navegador del usuario NO tiene — por eso "no salían las imágenes"
// aunque sí estén ahí en el HTML crudo del ticket.
//
// Esta función las detecta ANTES de que stripHtml() las borre junto con el
// resto de las etiquetas, y arma una URL propia (/api/glpi-image) que el
// navegador SÍ puede pedir: ese endpoint abre una sesión de GLPI del lado
// del servidor y reenvía la imagen. Las imágenes pegadas como data-URI
// (base64 inline) se dejan tal cual porque ya son autocontenidas.
function extractImages(html, system) {
  if (!html) return [];
  const images = [];
  ...
  for (const tag of html.match(imgTagRe) || []) {
    ...
    const rawSrc = decodeEntities(srcMatch[1]);
    const alt = decodeEntities(tag.match(altRe)?.[1] || "Imagen adjunta");

    if (rawSrc.startsWith("data:")) {
      // Ya autocontenida (pegada directo como base64) — se usa tal cual.
      images.push({ src: rawSrc, alt });
      continue;
    }

    // Se normaliza a una ruta relativa al propio GLPI (quitando el
    // baseUrl si vino absoluta), y solo se acepta si es el endpoint
    // conocido de descarga de documentos — cualquier otra cosa se
    // descarta, para que /api/glpi-image no se convierta en un proxy
    // abierto hacia URLs arbitrarias.
    let path = rawSrc;
    if (system.baseUrl && path.startsWith(system.baseUrl)) {
      path = path.slice(system.baseUrl.length);
    }
    if (!path.startsWith("/")) continue; // URL externa a otro host: se ignora
    if (!/^\/front\/document\.send\.php\?/i.test(path)) continue;

    images.push({
      src: `/api/glpi-image?system=${encodeURIComponent(system.id)}&path=${encodeURIComponent(path)}`,
      alt,
    });
  }
...
//
// Configurable por variable de entorno porque algunos GLPI (típicamente los
// que tienen más tickets, o están detrás de VPN) tardan más y con 15s
// fallaban de forma intermitente aunque el servidor sí estuviera respondiendo
// — solo lento. Ajusta GLPI_REQUEST_TIMEOUT_MS en .env.local si sigue
// pasando (p. ej. 45000 para 45s).
const GLPI_REQUEST_TIMEOUT_MS = Number(process.env.GLPI_REQUEST_TIMEOUT_MS) || 25_000;
function glpiFetch(url, options = {}) {
  return fetch(url, { ...options, signal: AbortSignal.timeout(GLPI_REQUEST_TIMEOUT_MS) });
}

// Reintenta una petición SOLO cuando el fallo parece transitorio (timeout,
// conexión reiniciada/rechazada a medias) — nunca ante una respuesta HTTP
// de error (4xx/5xx), porque esas no se arreglan solas reintentando. Pensado
// para la descarga de tickets de sistemas grandes (CNS, UnADM, Prepa), que
// por VPN a veces se cuelgan a medias una vez y a la siguiente sí responden.
async function glpiFetchWithRetry(url, options = {}, { retries = 2, backoffMs = 800 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await glpiFetch(url, options);
    } catch (err) {
      lastErr = err;
      const isTimeout = err.name === "TimeoutError";
      const code = err.cause?.code;
      const isTransientNetworkError = code === "ECONNRESET" || code === "ETIMEDOUT" || code === "EPIPE";
      if ((!isTimeout && !isTransientNetworkError) || attempt === retries) throw err;
      console.log(
        `[glpi] reintentando (${attempt + 1}/${retries}) tras ${describeGlpiError(err)}`
      );
...
async function fetchTicketsForSystem(system, { rangeSize = 200, maxTickets = 5000 } = {}) {
  let sessionToken;
  const t0 = Date.now();
  console.log(`[glpi:${system.id}] iniciando sesión…`);
  try {
    sessionToken = await initSession(system);
    console.log(`[glpi:${system.id}] sesión OK (${Date.now() - t0}ms) — pidiendo tickets…`);
    ...
    for (;;) {
      ...
      const res = await glpiFetchWithRetry(url.toString(), {
        method: "GET",
        headers,
        cache: "no-store",
        dispatcher: dispatcherFor(system),
      ...
      console.log(
        `[glpi:${system.id}] página ${page}: ${raw.length} tickets (${allRaw.length} acumulados, ${Date.now() - t0}ms transcurridos)`
      );
    ...
    console.log(`[glpi:${system.id}] LISTO: ${tickets.length} tickets en ${Date.now() - t0}ms`);
    ...
    const msg = describeGlpiError(err);
    console.log(`[glpi:${system.id}] FALLÓ tras ${Date.now() - t0}ms: ${msg}`);
    return {
      systemId: system.id,
...
async function fetchTicketExtrasBatch(system, rawIds) {
  ...
  try {
    ...
    // Cache de nombres/correo de usuario dentro de este lote, para no pedir
    // el mismo /User/{id} varias veces si aparece en varios tickets.
    const userInfoCache = new Map(); // uid -> { name, email } | null
    async function fetchUserInfo(uid) {
      ...
      try {
        ...
        if (uRes.ok) {
          ...
          const name = [uData.firstname, uData.realname].filter(Boolean).join(" ") || uData.name || null;
          info = { name, email: uData.email || null };
        }
      } catch {
        // no crítico
      }
    ...
    // Nombre completo (o null si no se pudo resolver) — para "personal que
    // atendió/resolvió", donde un ID sin nombre no aporta nada.
    async function userName(uid) {
      const info = await fetchUserInfo(uid);
      return info?.name || null;
    }
    // Nombre si está disponible, si no el correo — para "Solicitante": hay
    // instancias GLPI donde, por permisos del usuario de la API, el nombre
    // no viene pero el correo sí, y mostrar ALGO identificable es mejor que
    // dejar solo el ID numérico sin contexto.
    async function userDisplay(uid) {
      const info = await fetchUserInfo(uid);
      return info?.name || info?.email || null;
    }
      ...
      try {
        ...
        try {
          ...
          if (gtRes.ok) {
            ...
            let assignedIds = [
              ...new Set(
                groupList.filter((g) => Number(g.type) === 2).map((g) => g.groups_id).filter(Boolean)
              ),
            ];
            if (assignedIds.length === 0) {
              assignedIds = [...new Set(groupList.map((g) => g.groups_id).filter(Boolean))];
            }
        ...
        // Personal que atendió (Ticket_User type=2 = asignado) y
        // Solicitante (Ticket_User type=1) se resuelven de la MISMA
        // llamada a Ticket_User, para no duplicar la petición.
        try {
          ...
          if (tuRes.ok) {
            ...
            const assignedUserIds = [
              ...new Set(
                tuList.filter((u) => Number(u.type) === 2).map((u) => u.users_id).filter(Boolean)
              ),
            ];
            const names = await Promise.all(assignedUserIds.map(userName));
            extras.assignedStaff = names.filter(Boolean);

            // Puede haber más de un solicitante registrado; se muestra el
            // primero (nombre, o correo si el nombre no vino por permisos).
            const requesterIds = [
              ...new Set(
                tuList.filter((u) => Number(u.type) === 1).map((u) => u.users_id).filter(Boolean)
              ),
            ];
            if (requesterIds.length > 0) {
              extras.requesterDisplay = await userDisplay(requesterIds[0]);
            }
          }
        } catch {
          // sin personal asignado/solicitante disponible desde Ticket_User
        }
        ...
        try {
          ...
          if (solRes.ok) {
            ...
            solList.sort(
              (a, b) => new Date((b.date_creation || "").replace(" ", "T")) - new Date((a.date_creation || "").replace(" ", "T"))
            );
        ...
        // Respaldo si Ticket_User no trajo un solicitante (type=1) — por
        // ejemplo si esa instancia no expone ese subitem por permisos —
        // se intenta con el destinatario registrado en el ticket mismo.
        if (!extras.requesterDisplay && ticketRaw?.users_id_recipient) {
          extras.requesterDisplay = await userDisplay(ticketRaw.users_id_recipient);
        }
      } catch (err) {
  ...
  try {
    ...
    try {
      ...
      if (fRes.ok) {
        ...
        try {
          const legacyRes = await glpiFetch(
    ...
    return {
      ...
      // Imágenes pegadas/adjuntas en la descripción (ver comentario de
      // extractImages arriba: se resuelven vía /api/glpi-image porque el
      // navegador del usuario no tiene la sesión de GLPI para pedirlas
      // directo).
      images: extractImages(ticketRaw.content || "", system),
      groupNames,
      lastFollowup: lastFollowup
        ? {
            ...
            images: extractImages(lastFollowup.content || "", system),
