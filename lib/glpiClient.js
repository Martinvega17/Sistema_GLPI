...
async function fetchTicketDetail(system, rawId) {
  ...
  try {
    ...
    let attendedByAll = [];
    try {
      const uniqueAuthorIds = [...new Set(followups.map((f) => f.users_id).filter(Boolean))].slice(
        0,
        MAX_ATTENDERS
      );
      attendedByAll = (
        await Promise.all(
          uniqueAuthorIds.map(async (uid) => {
            ...
            try {
              ...
              const uData = await uRes.json();
              return [uData.firstname, uData.realname].filter(Boolean).join(" ") || uData.name || null;
            } catch {
              return null;
            }
          })
    ...
    try {
      const solRes = await fetch(
        `${system.baseUrl}/apirest.php/Ticket/${rawId}/ITILSolution?range=0-4`,
        opts
      );
      if (solRes.ok) {
          ...
          if (lastSolution.users_id_editor || lastSolution.users_id) {
            ...
            try {
              ...
              if (suRes.ok) {
                ...
                resolvedByName = [suData.firstname, suData.realname].filter(Boolean).join(" ") || suData.name || null;
              }
            } catch {
              // no crítico
            }
    ...
    return {
      ...
      attendedByAll: attendedByAll.length > 0 ? attendedByAll : authorName ? [authorName] : [],
